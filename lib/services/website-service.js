...
var remove = function(ws, callback) {
    ...
    Website.findOne({name : ws.name}, function(err, website) {
        ...
        if (website.user !== ws.user) {  // Si el usuario no es el mismo que registró el sitio web
            return callback(false, "No tienes permiso para eliminar este sitio web.");
        }
        website.remove(function(err) {
            if (err) {
                return callback(false, "Ha habido un error. Inténtelo de nuevo.");
            }
            // Si lo ha borrado, se busca el usuario y se decrementa el nº de sitios web registrados por él
            User.findOne({username : ws.user}, function(err, user) {
                if (err) {
            ...
            callback(true, "El usuario @" + ws.user + " ha eliminado el sitio web *" + ws.name + "*.");
        });
